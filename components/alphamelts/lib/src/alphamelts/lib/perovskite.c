const char *perovskite_ver(void) { return "$Id: perovskite.c,v 1.3 2006/10/20 00:59:22 ghiorso Exp $"; }
/*
MELTS Source Code: RCS $Log: perovskite.c,v $
...
**  MODULE DESCRIPTION:
**
**      Routines to compute perovskite solution properties 
**      (file: PEROVSKITE.C)
**
...
/*
 *=============================================================================
 * Perovskite solution parameters:
 *
 * Fei Y.
 ...
 *
 */
#define WH112 -4050.0   /* joules     */
#define WS112     1.45  /* joules/K   */
#define WV112     0.015 /* joules/bar */
#define WH122  4120.0   /* joules     */
#define WS122     1.27  /* joules/K   */
#define WV122     0.011 /* joules/bar */
 ...
 */
int
testPvk(int mask, double t, double p,
...
{
  const char *phase = "perovskite.c";
  const char *NAMES[NA]    = { "magnesioperovskite", "ferroperovskite" };
...
void
conPvk(int inpMask, int outMask, double t, double p,
  double *e,      /* comp of perovskite in moles of elements                  */
  double *m,      /* comp of perovskite in moles of endmember components      */
  double *r,      /* comp of perovskite in terms of the independent comp var  */
  double *x,      /* comp of perovskite in mole fractions of endmember comp   */
  ...
  double ****d3m) /* 3rd deriv matrix: d3m[i][j][k][l]=d3r[i]/dm[j]dm[k]dm[l] */
{
  ...
  (1) converts a vector of moles of elements into a vector of moles of 
      endmember perovskite components.
  ...
  numbers and that the order of perovskite components has been verified as:
        m[0] = magnesioperovskite (MgSiO3) and
        m[1] = ferroperovskite    (FeSiO3)
  ...
  if (inpMask == FIRST && outMask == SECOND) {
    ...
    if (outMask & ~(THIRD | FOURTH | FIFTH | SIXTH | EIGHTH))
      printf("Illegal call to conPvk with inpMask = %o and outMask = %o\n",
        inpMask, outMask);
...
void
dispPvk(int mask, double t, double p, double *x,
...
void 
actPvk(int mask, double t, double p, double *x, 
  ...
  )           /* exclusion criteria applied to results if BINARY MASK: 1000 */
{
  ...
  if (mask & FOURTH) {
    ...
    x[0] = xmg; /* mole fraction of magnesioperovskite                 */
    x[1] = xfe; /* mole fraction of ferroperovskite                    */
...
void 
gmixPvk(int mask, double t, double p, double *x, 
...
void 
hmixPvk(int mask, double t, double p, double *x, 
...
void 
smixPvk(int mask, double t, double p, double *x, 
...
void 
cpmixPvk(int mask, double t, double p, double *x, 
...
void 
vmixPvk(int mask, double t, double p, double *x, 
