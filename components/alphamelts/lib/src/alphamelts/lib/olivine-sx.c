const char *olivine_sx_ver(void) { return "$Id: olivine-sx.c,v 1.3 2006/10/20 00:59:22 ghiorso Exp $"; }
/*
MELTS Source Code: RCS $Log: olivine-sx.c,v $
...
MELTS Source Code: RCS
MELTS Source Code: RCS Revision 1.1  2002/04/06 00:51:39  ghiorso
...
**  MODULE DESCRIPTION:
**
**      Routines to compute olivine solution properties 
**      (file: OLIVINE-SX.C)
**
**  MODIFICATION HISTORY:
**
**      V1.0-1  Mark S. Ghiorso  April 5, 2002 
**
*/
...
/*
 *=============================================================================
 * (alpha) Olivine solution parameters:
 *
 * Fei Y.
 ...
 */
int
testOsx(int mask, double t, double p,
...
{
  const char *phase = "olivine-sx.c";
  const char *NAMES[NA]    = { "forsterite-sx", "fayalite-sx" };
...
void
conOsx(int inpMask, int outMask, double t, double p,
  double *e,      /* comp of olivine-sx in moles of elements                  */
  double *m,      /* comp of olivine-sx in moles of endmember components      */
  double *r,      /* comp of olivine-sx in terms of the independent comp var  */
  double *x,      /* comp of olivine-sx in mole fractions of endmember comp   */
  ...
  double ****d3m) /* 3rd deriv matrix: d3m[i][j][k][l]=d3r[i]/dm[j]dm[k]dm[l] */
{
  ...
  (1) converts a vector of moles of elements into a vector of moles of 
      endmember olivine-sx components.
  ...
  numbers and that the order of olivine-sx components has been verified as:
        m[0] = forsterite-sx (MgSi0.5O2) and
        m[1] = fayalite-sx   (FeSi0.5O2)
  ...
  if (inpMask == FIRST && outMask == SECOND) {
    ...
    if (outMask & ~(THIRD | FOURTH | FIFTH | SIXTH | EIGHTH))
      printf("Illegal call to conOsx with inpMask = %o and outMask = %o\n",
        inpMask, outMask);
...
void
dispOsx(int mask, double t, double p, double *x,
...
void 
actOsx(int mask, double t, double p, double *x, 
  ...
  )           /* exclusion criteria applied to results if BINARY MASK: 1000 */
{
  ...
  if (mask & FOURTH) {
    ...
    x[0] = xmg; /* mole fraction of forsterite-sx                      */
    x[1] = xfe; /* mole fraction of fayalite-sx                        */
...
void 
gmixOsx(int mask, double t, double p, double *x, 
...
void 
hmixOsx(int mask, double t, double p, double *x, 
...
void 
smixOsx(int mask, double t, double p, double *x, 
...
void 
cpmixOsx(int mask, double t, double p, double *x, 
...
void 
vmixOsx(int mask, double t, double p, double *x, 
