const char *ringwoodite_ver(void) { return "$Id: ringwoodite.c,v 1.3 2006/10/20 00:59:22 ghiorso Exp $"; }
/*
MELTS Source Code: RCS $Log: ringwoodite.c,v $
...
**  MODULE DESCRIPTION:
**
**      Routines to compute ringwoodite solution properties 
**      (file: RINGWOODITE.C)
**
...
/*
 *=============================================================================
 * Ringwoodite solution parameters:
 *
 * Fei Y.
 ...
 *
 */
#define WH112  3900.0   /* joules     */
#define WS112     1.10  /* joules/K   */
#define WV112     0.00  /* joules/bar */
#define WH122  3900.0   /* joules     */
 ...
 */
int
testRgw(int mask, double t, double p,
...
{
  const char *phase = "ringwoodite.c";
  const char *NAMES[NA]    = { "magnesioringwoodite", "ferroringwoodite" };
...
void
conRgw(int inpMask, int outMask, double t, double p,
  double *e,      /* comp of ringwoodite in moles of elements                 */
  double *m,      /* comp of ringwoodite in moles of endmember components     */
  double *r,      /* comp of ringwoodite in terms of the independent comp var */
  double *x,      /* comp of ringwoodite in mole fractions of endmember comp  */
  ...
  double ****d3m) /* 3rd deriv matrix: d3m[i][j][k][l]=d3r[i]/dm[j]dm[k]dm[l] */
{
  ...
  (1) converts a vector of moles of elements into a vector of moles of 
      endmember ringwoodite components.
  ...
  numbers and that the order of ringwoodite components has been verified as:
        m[0] = magnesioringwoodite (MgSi0.5O2) and
        m[1] = ferroringwoodite    (FeSi0.5O2)
  ...
  if (inpMask == FIRST && outMask == SECOND) {
    ...
    if (outMask & ~(THIRD | FOURTH | FIFTH | SIXTH | EIGHTH))
      printf("Illegal call to conRgw with inpMask = %o and outMask = %o\n",
        inpMask, outMask);
...
void
dispRgw(int mask, double t, double p, double *x,
...
void 
actRgw(int mask, double t, double p, double *x, 
  ...
  )           /* exclusion criteria applied to results if BINARY MASK: 1000 */
{
  ...
  if (mask & FOURTH) {
    ...
    x[0] = xmg; /* mole fraction of magnesioringwoodite                */
    x[1] = xfe; /* mole fraction of ferroringwoodite                   */
...
void 
gmixRgw(int mask, double t, double p, double *x, 
...
void 
hmixRgw(int mask, double t, double p, double *x, 
...
void 
smixRgw(int mask, double t, double p, double *x, 
...
void 
cpmixRgw(int mask, double t, double p, double *x, 
...
void 
vmixRgw(int mask, double t, double p, double *x, 
