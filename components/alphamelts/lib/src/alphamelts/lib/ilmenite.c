const char *ilmenite_ver(void) { return "$Id: ilmenite.c,v 1.3 2006/10/20 00:59:22 ghiorso Exp $"; }
/*
MELTS Source Code: RCS $Log: ilmenite.c,v $
...
MELTS Source Code: RCS
MELTS Source Code: RCS Revision 1.1  2002/04/05 01:40:20  ghiorso
...
**  MODULE DESCRIPTION:
**
**      Routines to compute silicate ilmenite solution properties 
**      (file: ILMENITE.C)
**
 ...
 *
 */
#define WH112     0.0   /* joules     */
#define WS112     0.0   /* joules/K   */
#define WV112     0.0   /* joules/bar */
#define WH122     0.0   /* joules     */
#define WS122     0.0   /* joules/K   */
#define WV122     0.0   /* joules/bar */
 ...
 */
int
testIlm(int mask, double t, double p,
...
{
  const char *phase = "ilmenite.c";
  const char *NAMES[NA]    = { "magnesioilmenite", "ferroilmenite" };
  const char *FORMULAS[NA] = { "MgSiO3", "FeSiO3" };
  int result = TRUE, i;
  double sum;
...
void
conIlm(int inpMask, int outMask, double t, double p,
  double *e,      /* comp of ilmenite in moles of elements                  */
  double *m,      /* comp of ilmenite in moles of endmember components      */
  double *r,      /* comp of ilmenite in terms of the independent comp var  */
  double *x,      /* comp of ilmenite in mole fractions of endmember comp   */
  ...
  double ****d3m) /* 3rd deriv matrix: d3m[i][j][k][l]=d3r[i]/dm[j]dm[k]dm[l] */
{
  ...
  numbers and that the order of ilmenite components has been verified as:
        m[0] = magnesioilmenite (MgSiO3) and
        m[1] = ferroilmenite    (FeSiO3)
  ...
  if (inpMask == FIRST && outMask == SECOND) {
    ...
    m[0] = e[Mg]; /* moles of MgSiO3                   */
    m[1] = e[Fe]; /* Moles of FeSiO3                   */

  } else if (inpMask == SECOND) {
    double sum;

    if (outMask & ~(THIRD | FOURTH | FIFTH | SIXTH | EIGHTH))
      printf("Illegal call to conIlm with inpMask = %o and outMask = %o\n",
        inpMask, outMask);
...
void
dispIlm(int mask, double t, double p, double *x,
...
void 
actIlm(int mask, double t, double p, double *x, 
  ...
  )           /* exclusion criteria applied to results if BINARY MASK: 1000 */
{
  ...
  if (mask & FOURTH) {
    ...
    static const double exclusion[NA] = {
       0.05,  /* exclusion criteria on the mole fraction of MgSiO3     */
       0.05   /* exclusion criteria on the mole fraction of FeSiO3     */
    };
    double x[NA];

    x[0] = xmg; /* mole fraction of magnesioilmenite                 */
    x[1] = xfe; /* mole fraction of ferroilmenite                    */
...
void 
gmixIlm(int mask, double t, double p, double *x, 
...
void 
hmixIlm(int mask, double t, double p, double *x, 
...
void 
smixIlm(int mask, double t, double p, double *x, 
...
void 
cpmixIlm(int mask, double t, double p, double *x, 
...
void 
vmixIlm(int mask, double t, double p, double *x, 
