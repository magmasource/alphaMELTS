const char *wustite_ver(void) { return "$Id: wustite.c,v 1.3 2006/10/20 00:59:23 ghiorso Exp $"; }
/*
MELTS Source Code: RCS $Log: wustite.c,v $
...
**  MODULE DESCRIPTION:
**
**      Routines to compute wustite solution properties 
**      (file: WUSTITE.C)
**
 ...
 *
 */
#define WH112 16100.0   /* joules     */
#define WS112     0.00  /* joules/K   */
#define WV112     0.00  /* joules/bar */
#define WH122 26300.0   /* joules     */
#define WS122     5.56  /* joules/K   */
 ...
 */
int
testWus(int mask, double t, double p,
...
{
  const char *phase = "wustite.c";
  const char *NAMES[NA]    = { "magnesiowustite", "ferrowustite" };
  const char *FORMULAS[NA] = { "MgO", "FeO" };
  int result = TRUE, i;
  double sum;
...
void
conWus(int inpMask, int outMask, double t, double p,
  double *e,      /* comp of wustite in moles of elements                     */
  double *m,      /* comp of wustite in moles of endmember components         */
  double *r,      /* comp of wustite in terms of the independent comp var     */
  double *x,      /* comp of wustite in mole fractions of endmember comp      */
  ...
  double ****d3m) /* 3rd deriv matrix: d3m[i][j][k][l]=d3r[i]/dm[j]dm[k]dm[l] */
{
  ...
  numbers and that the order of wustite components has been verified as:
        m[0] = magnesiowustite (MgO) and
        m[1] = ferrowustite    (FeO)
  ...
  if (inpMask == FIRST && outMask == SECOND) {
    ...
    m[0] = e[Mg]; /* moles of MgO                      */
    m[1] = e[Fe]; /* Moles of FeO                      */

  } else if (inpMask == SECOND) {
    double sum;

    if (outMask & ~(THIRD | FOURTH | FIFTH | SIXTH | EIGHTH))
      printf("Illegal call to conWus with inpMask = %o and outMask = %o\n",
        inpMask, outMask);
...
void
dispWus(int mask, double t, double p, double *x,
...
void 
actWus(int mask, double t, double p, double *x, 
  ...
  )           /* exclusion criteria applied to results if BINARY MASK: 1000 */
{
  ...
  if (mask & FOURTH) {
    ...
    static const double exclusion[NA] = {
       0.05,  /* exclusion criteria on the mole fraction of MgO        */
       0.05   /* exclusion criteria on the mole fraction of FeO        */
    };
    double x[NA];

    x[0] = xmg; /* mole fraction of magnesiowustite                    */
    x[1] = xfe; /* mole fraction of ferrowustite                       */
...
void 
gmixWus(int mask, double t, double p, double *x, 
...
void 
hmixWus(int mask, double t, double p, double *x, 
...
void 
smixWus(int mask, double t, double p, double *x, 
...
void 
cpmixWus(int mask, double t, double p, double *x, 
...
void 
vmixWus(int mask, double t, double p, double *x, 
