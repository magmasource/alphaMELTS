const char *clinoamphibole_ver(void) { return "$Id: amphibole.c,v 1.3 2006/10/20 00:59:22 ghiorso Exp $"; }
/*
 ...
 *    The function conCph defines the conversion from m[i], to r[j]
 */
                   /* Order: r, p */
       ...
       (r[0] != rOld[0]) || (r[1] != rOld[1]) ) {
        ...
        for (i=0; i<NS; i++) {
            if (dgds[i] > sqrt(DBL_EPSILON) && ABS(sOld[i]) > DBL_EPSILON) {
                printf("ERROR in AMPHIBOLE.C (function ORDER). Failed to converge!\n");
 ...
 */
int
testCph(int mask, double t, double p,
...
{
    const char *phase = "clinoamphibole.c";
...
void
conCph(int inpMask, int outMask, double t, double p,
...
*/
        ...
        if (outMask & ~(THIRD | FOURTH | FIFTH | SIXTH | EIGHTH))
            printf("Illegal call to conCph with inpMask = %o and outMask = %o\n",
                inpMask, outMask);
...
void
dispCph(int mask, double t, double p, double *x,
...
void
actCph(int mask, double t, double p, double *x,
...
void
gmixCph(int mask, double t, double p, double *x,
...
void
hmixCph(int mask, double t, double p, double *x,
...
void
smixCph(int mask, double t, double p, double *x,
...
void
cpmixCph(int mask, double t, double p, double *x,
...
void
vmixCph(int mask, double t, double p, double *x,
