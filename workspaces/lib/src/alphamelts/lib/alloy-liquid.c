const char *alloy_liquid_ver(void) { return "$Id: alloy-liquid.c,v 1.3 2006/10/20 00:59:22 ghiorso Exp $"; }
/*
MELTS Source Code: RCS $Log: alloy-liquid.c,v $
 ...
 */
int
testAlL(int mask, double t, double p,
...
{
  const char *phase = "alloy-liquid.c";
...
void
conAlL(int inpMask, int outMask, double t, double p,
  ...
  double ****d3m) /* 3rd deriv matrix: d3m[i][j][k][l]=d3r[i]/dm[j]dm[k]dm[l] */
{
  ...
  if (inpMask == FIRST && outMask == SECOND) {
    ...
    if (outMask & ~(THIRD | FOURTH | FIFTH | SIXTH | EIGHTH))
      printf("Illegal call to conAlL with inpMask = %o and outMask = %o\n",
        inpMask, outMask);
...
void
dispAlL(int mask, double t, double p, double *x,
...
/*             1111111111222222222233333333334444444444555555555566666666667
  ...
  if (mask & FIRST) {
    ...
    (void) snprintf(n, 5, "%4.2f", totFe); for (i=0; i<4; i++) string[ 9+i] = n[i];
    (void) snprintf(n, 5, "%4.2f", totNi); for (i=0; i<4; i++) string[15+i] = n[i];
...
void 
actAlL(int mask, double t, double p, double *x, 
...
void 
gmixAlL(int mask, double t, double p, double *x, 
...
void 
hmixAlL(int mask, double t, double p, double *x, 
...
void 
smixAlL(int mask, double t, double p, double *x, 
...
void 
cpmixAlL(int mask, double t, double p, double *x, 
...
void 
vmixAlL(int mask, double t, double p, double *x, 
